...
def decrypt_file(file_path, xxtea_key, xxtea_encrypt):
    with open(file_path, 'rb') as f:
        if args.xor_key:
            ...
            data = f.read()
    if data and file_path.endswith('.js'):
        data_len = len(data)
        key = xxtea_key
        key_len = len(key)
        ...
        if ret_len.value > 0:
            if decrypted_data[:2] == b"PK":
                ...
                write_file(file_path.replace('.js','.jss'), zlib.decompress(decrypted_data, 16 + zlib.MAX_WBITS))
            else:
                try:
                    write_file(file_path.replace('.js','.jsc'), decrypted_data)
...
def decrypt_folder(path, xxtea_key, xxtea_encrypt):
    if os.path.isdir(path):
        for root, dirs, files in os.walk(path):
            for file in files:
                file_path = os.path.join(root, file)
                if args.xor_key or file_path.endswith('js'):
                    decrypt_file(file_path, xxtea_key, xxtea_encrypt)
    else:
        if args.xor_key or file_path.endswith('js'):
            decrypt_file(path, xxtea_key, xxtea_encrypt)
...
def main():
    ...
    xxtea_encrypt = my_lib.xxtea_encrypt
    xxtea_encrypt.argtypes = [c_char_p, c_ulong, c_char_p, c_ulong, ctypes.POINTER(c_ulong)]
    xxtea_encrypt.restype = ctypes.POINTER(ctypes.c_ubyte)

    decrypt_folder(args.path, args.key.encode(), xxtea_encrypt)
